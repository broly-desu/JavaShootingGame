...
public class GameFrame extends MyFrame{
	public void run() {
		...
		while(true) {
			...
			if(GameWorld.stage%5 == 0 && GameWorld.stage != 0) {
				GameWorld.enemies.add(new BossEnemyBase(100,50,GameWorld.stage,0));
				GameWorld.enemies.add(new EnemyBase(170,100,GameWorld.stage,0));
				GameWorld.enemies.add(new EnemyBase(30,100,GameWorld.stage,0));
	...
	public void checkPlayerAndEnemies() {
		for (int i=0;i<GameWorld.enemies.size();i++) {
			Enemy e=GameWorld.enemies.get(i);
			if(checkHit(GameWorld.player,e,true)) {
	...
	public boolean checkHit(Character a,Character b,boolean c) {
		if(c)
		{
